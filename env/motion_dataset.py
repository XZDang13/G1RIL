...
class MotionLoader:
    ...
    def __init__(self, motion_file: str, device: torch.device) -> None:
        ...
        self.dof_positions = torch.tensor(data["dof_positions"][:300], dtype=torch.float32, device=self.device)
        self.dof_velocities = torch.tensor(data["dof_velocities"][:300], dtype=torch.float32, device=self.device)
        self.body_positions = torch.tensor(data["body_positions"][:300], dtype=torch.float32, device=self.device)
        self.body_rotations = torch.tensor(data["body_rotations"][:300], dtype=torch.float32, device=self.device)
        self.body_linear_velocities = torch.tensor(
            data["body_linear_velocities"][:300], dtype=torch.float32, device=self.device
        )
        self.body_angular_velocities = torch.tensor(
            data["body_angular_velocities"][:300], dtype=torch.float32, device=self.device
        )
